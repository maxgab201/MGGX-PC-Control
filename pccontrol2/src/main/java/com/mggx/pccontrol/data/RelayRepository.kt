...
class HttpRelayApi(private val config: RelayConfig, private val clientOverride: OkHttpClient? = null) : PcRelayApi {
    ...
    suspend fun getHealth(): RelayResult<RelayHealth> = request("health", authenticated = false).flatMap { body, code, time -> runCatching { val json = JSONObject(body); val version = json.optInt("version", -1); require(json.optBoolean("ok", false) && json.optString("service") == "mggx-relay" && version >= 1); RelayResult.Success(RelayHealth(version), code, time) }.getOrElse { RelayResult.Failure(RelayError.InvalidResponse, RelayTechnical("HEALTH_PARSE", elapsedMs = time)) } }
