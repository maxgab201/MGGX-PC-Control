...
import com.mggx.pccontrol.next.domain.ConnectionDiagnostics
import com.mggx.pccontrol.next.domain.DiagnosticStage
import com.mggx.pccontrol.next.domain.DiagnosticState
...
import com.mggx.pccontrol.next.domain.userMessage
    ...
    suspend fun run(config: RelayConfig, onUpdate: (ConnectionDiagnostics) -> Unit = {}): ConnectionDiagnostics {
        ...
        when (val status = api.getStatus()) {
            is RelayResult.Success -> {
                val pc = status.value
                val agent = when {
                    pc.state == com.mggx.pccontrol.next.domain.PcState.OFFLINE -> DiagnosticState.SKIPPED to "PC apagada"
